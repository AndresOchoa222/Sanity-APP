...
# Credenciales tomadas de st.secrets
...
def login() -> None:
    """Mostrar formulario de inicio de sesión y validar credenciales."""
...
# Definiciones de columnas en los datos
...
# Etiquetas y configuración de transacciones
...
def read_table(path: str | Path) -> pd.DataFrame:
    """Cargar CSV o Excel detectando el separador de forma automática desde disco."""
    file_path = Path(path)
...
def read_uploaded_file(file_bytes: bytes, filename: str) -> pd.DataFrame:
    """Leer un archivo subido (CSV o Excel) a partir de sus bytes."""
...
def normalize_frame(df: pd.DataFrame) -> pd.DataFrame:
    """Normalizar columnas clave y convertir fechas a datetime."""
...
def _oldest_in_progress(per_method: pd.DataFrame) -> Dict[str, str]:
    """Obtener la fecha más antigua en estado IN_PROGRESS por método."""
...
def _build_pivot(table: pd.DataFrame) -> pd.DataFrame:
    """Construir tabla dinámica de recuentos por método y estado."""
    pivot = (
...
def plot_heatmap(data: pd.DataFrame, country_code: int, trx_type: str, *, show: bool = True) -> Optional[plt.Figure]:
    """Generar un heatmap y opcionalmente mostrarlo en pantalla."""
    ...
    if uploaded_file is not None:
        ...
        for code_int in codes:
            ...
            for trx_type in TYPE_ORDER:
                ...
                if fig is not None:
                    st.pyplot(fig)
                    palette = TYPE_CMAP.get(trx_type, "BuGn")
                    st.caption(
                        f"Este heatmap usa la paleta de colores {palette} "
                        f"para el tipo de transacción {trx_type.lower()}. "
                        "El eje horizontal muestra los estados de la transacción y el eje vertical los métodos de pago. "
                        "La intensidad del color en cada celda representa la cantidad de transacciones: cuanto más oscuro, mayor es el número. "
                        "Las etiquetas de cada celda indican el total de transacciones y, si el estado es IN_PROGRESS, la fecha y hora más antiguas registradas."
                    )
                    st.markdown("---")

# Ejecutar la app según el estado de autenticación
